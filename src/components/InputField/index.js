const InoutField = ({
  ...
  return (
    <div className={`floating-input w-full ${error || timer ? 'mb-1' : 'mb-5'} relative`}>
      <input
        type={type}
        // id={name}
        name={name}
      ...
      <div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none">{svg}</div>
    </div>
  );
};
